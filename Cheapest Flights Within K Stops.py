...
class Solution(object):
    ...
    def findCheapestPriceOld(self, n, flights, src, dst, K):
        self.n = n
        self.src = src
    ...
    def findCheapestPrice(self, n, flights, src, dst, K):
        ...
        visited = {}
        for edge in flights:
            mapps[edge[0]].append([edge[1], edge[2]])
        # print(mapps[src])
        tmp = mapps[src]
        rtn = float('inf')
        while len(tmp)>0 and K >= 0:
            ...
            tmp_ =[]
            for each in tmp:
                key, dist = each[0], each [1]
                if key == dst:
                    rtn = min(dist, rtn)
                for maps in mapps[key]:
                    if (maps[0] in visited and visited[maps[0]] > maps[1]+dist) or maps[0] not in visited:
                        tmp_.append([maps[0], maps[1]+dist])
                        visited[key] = dist
                    else:
                        pass
            tmp = tmp_
